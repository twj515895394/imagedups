...
def input_path():
    ipath = input("请输入图片所在文件夹路径:  ").replace("\\", "\\\\")
...
if __name__ == '__main__':
    ###说明###
    print('[ ============================ ]')
    print('[  图片去重工具v.1.0.1 by 刺猬粑粑 ]')
    print('[ dfldata.xyz【论坛专用】 ]')
    print(r'''     _  __ _     _       _                         
        | |/ _| |   | |     | |                        
    ...
    | (_| | | | | (_| | (_| | || (_| |_ >  <| |_| |/ / 
     \__,_|_| |_|\__,_|\__,_|\__\__,_(_)_/\_\\__, /___|
                                             ...
                                             |___/     ''')
    print('[ ============================ ]')
    print('[ 本工具作用：该工具主要用于src图片去重 ]')
    print('[ 切src头像图片后，因为每帧可能会切好几张，导致重复图片过多，过多重复图片会给训练增加负担 ]')
    print('[ 注意事项：只适用于处理src重复图片的筛选移除 ]')
    print('[ ============================ ]')
    print('[ 使用说明： ]')
    print('[ 1.输入人脸图片的路径，如D:/人脸图片路径  ]')
    print('[ 2.选择模式，1 懒散模式(推荐)，2 精细模式。 实际算法不通 相差不大]')
    print('[ 3.比如a,b图片90%一样，但是可能有些细节，比如眼神看的方向是不一样的  ]')
    print(
        '[   精细模式会认为这是两张不同图片选择保留，而懒散模式会认为这两张图片相同，会选择删除后一张图片  ]')
    print('[ 4.被移除的图片会保存在上面输入的目录下的delete文件夹中。 ]')
    print('[ 5.源码开放的，报毒的话那就是杀毒软件有病！！！ ]')
    print('[ ============================ ]')
